...
export const getLibrariesForUser = catchAsync(async (req: Request, res: Response) => {
  ...
  const { page = 1, limit = 10, search = '' } = req.query;
  const libraries = await getLibrariesByManager(req.params["workspaceId"] as string,req.headers['accountid'] as string || userId || '', Number(page), Number(limit), search as string);
  res.status(200).json(libraries);
});
